...
public class RecommendationCommentController {
    ...
    public ResponseEntity<RecommendationCommentDto> addComment(@PathVariable Long id, @RequestBody RecommendationCommentDto commentDto) {
        return ResponseEntity.ok(recommendationCommentService.addComment(id, commentDto));
    }

    @GetMapping("/comments/{comment_id}")
    public ResponseEntity<RecommendationCommentDto> getComments(@PathVariable Long comment_id) {
        return ResponseEntity.ok(recommendationCommentService.getComment(comment_id));
    }

    @GetMapping("/{id}/comments")
    public ResponseEntity<List<RecommendationCommentDto>> getCommentsByRecommendationId(@PathVariable Long id) {
        List<RecommendationCommentDto> recommendationCommentDtos = recommendationCommentService.getCommentsByBoardId(id);
